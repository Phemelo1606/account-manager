...
function processTransaction(type, sourceAccountId, amount, destinationAccountId, currentTime, findAccount) {
    ...
    if (type === "withdrawal" || type === "transfer") {
        ...
        sourceAccount.withdrawalTimestamps = sourceAccount.withdrawalTimestamps.filter((timestamp) => timestamp >= currentTime - WITHDRAWAL_WINDOW_MS);
    ...
    sourceAccount.averageTransactionAmount = averageSuccessfulAmount(sourceAccount.transactionHistory);
    if (type === "transfer") {
    ...
    try {
        while (true) {
            ...
            if (choice === "1") {
                ...
                const result = processTransaction(type, sourceAccountId, amount, destinationAccountId, Date.now(), findAccount);
